  ...
  def apply[F[_]: Sync: Parallel](
    ...
    new Operation[F] {
      def run(ws: Workspace[F], log: Logger[F], b: Blocker): F[ExitCode] =
        new EmailImpl[F](siteConfig, ws, log).run
    }

}

private[operation] final class EmailImpl[F[_]: Sync: Parallel](
  siteConfig: Path,
  ws:         Workspace[F],
  log:        Logger[F]
) {
  ...
  val run: F[ExitCode] =
    for {
      ...
      ps <- ws.proposals
      _  <- createSuccessfulEmailsForClassical(ps, qc)
      // TODO: more!
  ...
  def createPiEmail(p: Proposal): F[MailMessage] = {
    ...
    for {
      ...
      tit = merge(t.name, t.titleTemplate, ps)
      _  <- Sync[F].delay(println(tit))
      bod = merge(t.name, t.bodyTemplate, ps)
  ...
  def createSuccessfulEmailsForClassical(ps: List[Proposal], qc: QueueConfig): F[List[MailMessage]] =
    ps.classicalProposalsForSite(qc.site).parFlatTraverse { cp =>
      (createPiEmail(cp), createNgoEmails(cp)).parMapN(_ :: _)
    }
   ...
   */
  def merge(templateName: String, template: String, bindings: Map[String, AnyRef]): Either[Throwable, String] =
    Either.catchNonFatal {
