...
public final class NightIterator implements Iterator<Night> {
    ...
    private final edu.gemini.spModel.core.Site siteDesc;
    ...
    public NightIterator(edu.gemini.spModel.core.Site site, Date start, Date end) {
        this(site, start, end, DEFAULT);
    }
     ...
     * @param end end date at which iteration stops
     */
    public NightIterator(edu.gemini.spModel.core.Site site, Date start, Date end, TwilightBoundType type) {
    ...
    // start and end times.
    private static Night firstNight(edu.gemini.spModel.core.Site site, long s, long e, TwilightBoundType type) {
        if (e <= s) return null;
